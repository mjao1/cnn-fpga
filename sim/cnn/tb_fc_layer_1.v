...
module tb_fc_layer_1;
    ...
    initial begin
        ...
        // Generate test inputs (simple ramp pattern)
        for (i = 0; i < IN_FEATURES; i = i + 1) begin
            test_inputs[i] = (i % 256) - 128; // -128 to 127
        end
        ...
        $display("Sending input data to FC1 layer...");
        ...
        wait(done_out);
        $display("FC1 layer signaled done_out=1 at time %t", $time);
        ...
        $display("FC1 layer signaled done_out=1 after reset at time %t", $time);
