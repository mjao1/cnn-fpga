...
module tb_pool_layer_1;
    ...
    initial begin
        ...
        // Test patterns for each channel
        for (ch = 0; ch < NUM_CHANNELS; ch = ch + 1) begin
            for (i = 0; i < IN_HEIGHT; i = i + 1) begin
                for (j = 0; j < IN_WIDTH; j = j + 1) begin
                    test_feature_maps[ch][i][j] = $signed({1'b0, ch[2:0], 1'b0, i[1:0], j[1:0]});
                    
                    if (ch >= 3) begin
                        if ((i + j) % 4 == 0) begin
                            test_feature_maps[ch][i][j] = $signed({1'b1, ch[2:0], 1'b0, i[1:0], j[1:0]});
                        end
                    end
                end
            end
        ...
        $display("=== Pool Layer 1 Testbench ===");
